...
저는 스마트관광 플랫폼에서 예약부터 결제까지 실제 운영되는 중개 서비스를 만들어봤고, JSP를 Next.js로 마이그레이션하면서 성능도 Lighthouse로 측정해 LCP를 60% 개선해봤습니다. 화면을 만드는 데서 끝내지 않고, 제가 짠 코드를 타입과 테스트로 직접 검증하는 걸 중요하게 생각합니다.

클로드 코드 같은 AI 도구도 업무에 적극적으로 활용하는데, 생성된 코드도 직접 읽고 검증한 뒤에 반영합니다.

리액트 네이티브는 예전에 해본 경험은 있지만 오래돼서, 하게 된다면 다시 공부하면서 진행해야 할 것 같습니다. 다만 리액트 개발자로 입사했는데도 회사 상황에 맞춰 JSP 프로젝트를 맡아 끝까지 해낸 적이 있어서, 필요한 스택은 공부해서 책임지고 마무리하는 편입니다.

이런 경험들을 바탕으로 빠르게 제품 개발에 기여하고 싶습니다. 감사합니다.`;
