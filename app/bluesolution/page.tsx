...
const GROUPS: Group[] = [
  ...
  },
  {
    ...
    items: [
      ...
      {
        q: "왜 어드민을 Next.js로? (실제 나옴 ★)",
        points: [
          "어드민은 SEO 불필요 → SSR 노린 게 아님(이걸 아는 게 핵심)",
          "유저 사이트가 이미 Next.js → 스택 통일로 일관성·유지보수",
          "모노레포로 두 앱 함께 관리 + packages/shared로 공통 컴포넌트·타입 공유",
          "파일 기반 라우팅·규격화된 구조로 빠른 개발",
        ],
      },
      {
        q: "Blue-Green 배포란? (실제 나옴, 못 답함 ★)",
        points: [
          "동일 운영환경 2개: Blue(현재 운영) + Green(새 버전)",
          "Green에 배포·검증 후 트래픽을 한 번에 전환 → 무중단",
          "문제 시 트래픽만 Blue로 되돌림 → 롤백 빠름 (단점: 리소스 2배)",
          "vs 롤링(조금씩 교체) / vs 카나리(일부 트래픽 점진). 우리는 ArgoCD로 이미지 태그 관리",
          "⚠️ 인프라/BE가 세팅했으면 '개념은 ~, 저는 파이프라인에 맞춰 작업'으로 솔직히",
        ],
      },
      {
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        q: "이벤트 루프 / 동기·비동기 ★단골",
        points: [
          "JS 엔진은 싱글 스레드(한 번에 하나). 비동기(setTimeout·fetch)는 브라우저(Web API)가 대신 처리",
          "끝나면 콜백이 큐에 쌓임 → 이벤트 루프가 콜스택 비었는지 확인 → 비면 큐에서 꺼내 실행",
          "마이크로태스크(Promise.then) > 매크로태스크(setTimeout) 우선",
          "예: console.log(1); setTimeout(..2..,0); Promise.then(..3..); console.log(4) → 출력 1,4,3,2",
          "비유: 요리사(JS)+주방보조(브라우저). 오래 걸리는 건 보조에 맡기고, 손 비면 대기줄에서 꺼냄",
        ],
      },
      {
        q: "this 바인딩",
  ...
  },
  {
    ...
    items: [
      ...
      {
        q: "리플로우 vs 리페인트 ★내 CLS와 연결",
        points: [
          "리플로우=크기·위치 변경으로 레이아웃 재계산(주변까지 연쇄, 비쌈) / 리페인트=색 등 시각만(가벼움)",
          "리플로우 생기면 리페인트도 따라옴 → 리플로우 최소화가 성능 핵심",
          "★내 경험: 이미지·배너에 크기 미지정 → 로드 시 레이아웃 밀림 → 리플로우 → CLS 악화",
          "→ width/height·aspect-ratio로 영역 예약해 밀림 제거, CLS 100% 개선",
        ],
      },
      {
        q: "이벤트 버블링 / 캡처링 / 위임",
