...
const WhyDynamicForm = () => (
  <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-6 mb-8 border border-blue-100">
    ...
    <div className="grid md:grid-cols-2 gap-6">
      <div>
        {[
          { num: "1", title: "프로젝트마다 다른 요구사항", desc: "전시회 A는 출입증에 차량번호 필요, 전시회 B는 불필요 등 프로젝트별 폼 구성이 상이", color: "blue" },
          { num: "2", title: "반복되는 유사 페이지", desc: "출입증, 독립부스, 가구비품, 홍보자료 등 10개 이상의 신청 페이지가 비슷한 폼 구조", color: "blue" },
        ].map((item) => (
          <div key={item.num} className="flex items-start gap-3 mb-4">
            <div className={`w-8 h-8 bg-${item.color}-100 rounded-lg flex items-center justify-center flex-shrink-0`}>
              <span className={`text-${item.color}-600 font-bold`}>{item.num}</span>
            </div>
            <div>
      ...
      <div>
        {[
          { num: "3", title: "백엔드와 협업 설계", desc: "API 스펙 협의를 통해 itemTypeCd, metaData 등 프론트 렌더링에 필요한 정보를 API에 포함", color: "green" },
          { num: "4", title: "사용자 경험 통일", desc: "프로젝트가 달라도 동일한 UX, 필드 레이아웃, 유효성 검사 방식 제공", color: "green" },
        ].map((item) => (
          <div key={item.num} className="flex items-start gap-3 mb-4">
            <div className={`w-8 h-8 bg-${item.color}-100 rounded-lg flex items-center justify-center flex-shrink-0`}>
              <span className={`text-${item.color}-600 font-bold`}>{item.num}</span>
    ...
    <div className="mt-4 p-3 bg-white rounded-lg border border-blue-200">
      <p className="text-sm text-gray-700 text-center">
        ...
        <strong className="text-blue-600"> 프론트엔드 코드 수정 0%</strong>
      </p>
    </div>
  </div>
);
  ...
  <>
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
      {fieldTypes.map((field) => (
        <div key={field.type} className="bg-gray-50 rounded-lg p-4 text-center hover:bg-gray-100 transition-colors">
    ...
    <div className="bg-gray-50 rounded-xl p-6 mb-8">
      <div className="flex flex-col lg:flex-row items-stretch gap-4">
        ...
        <div className="flex items-center justify-center lg:flex-col">
          <svg className="w-8 h-8 text-blue-500 rotate-90 lg:rotate-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  <>
    ...
    <div className="bg-blue-50 rounded-xl p-6 mb-8">
      <p className="text-sm text-gray-700 mb-4">
        ...
        API에서는 각 필드를 개별 관리하지만, <strong className="text-blue-600">사용자에게는 하나의 통합된 주소 입력 UI</strong>를 제공합니다.
      </p>
      <div className="grid md:grid-cols-2 gap-6">
        <div className="bg-white rounded-lg p-4 border border-red-200">
          <div className="text-sm font-bold text-red-600 mb-3 flex items-center gap-2">
            <span>기존 방식</span>
            <span className="text-xs bg-red-100 px-2 py-0.5 rounded">비효율</span>
          </div>
        ...
        <div className="bg-white rounded-lg p-4 border border-green-200">
          <div className="text-sm font-bold text-green-600 mb-3 flex items-center gap-2">
            <span>DynamicForm 방식</span>
            <span className="text-xs bg-green-100 px-2 py-0.5 rounded">해결</span>
          </div>
          <div className="space-y-2 text-sm text-gray-600">
            <p>• ZIPCODE 타입 하나로 <strong>통합 처리</strong></p>
            <p>• 관련 필드 자동 감지 (addr, addrDetail, addrEn)</p>
            <p>• 우편번호 검색 시 자동 채움</p>
            <p>• <strong className="text-green-600">사용자는 검색 한 번으로 완료</strong></p>
          </div>
        </div>
      </div>
      <div className="mt-6 bg-white rounded-lg p-4 border border-blue-200">
        ...
        <div className="space-y-2">
          <div className="flex gap-2">
            ...
            <button className="px-3 py-1 bg-blue-600 text-white text-xs rounded">우편번호 검색</button>
          </div>
    ...
    <div className="bg-purple-50 rounded-xl p-6 mb-8">
      <div className="grid md:grid-cols-2 gap-6">
        <div className="bg-white rounded-lg p-4 border border-purple-200">
          <div className="text-sm font-bold text-purple-600 mb-3">RADIO + TEXT_OPTION</div>
          <div className="space-y-3">
          ...
          <div className="mt-3 p-2 bg-purple-50 rounded text-xs font-mono">
            value: <span className="text-purple-600">&quot;other|커스텀부스&quot;</span>
          </div>
        </div>
        <div className="bg-white rounded-lg p-4 border border-purple-200">
          <div className="text-sm font-bold text-purple-600 mb-3">CHECKBOX + TEXT_OPTION</div>
          <div className="space-y-3">
            <label className="flex items-center gap-2">
          ...
          <div className="mt-3 p-2 bg-purple-50 rounded text-xs font-mono">
            value: <span className="text-purple-600">[&quot;electric&quot;, &quot;internet&quot;, &quot;other|추가장비&quot;]</span>
          </div>
        </div>
      </div>
    ...
    <div className="bg-orange-50 rounded-xl p-6 mb-8">
      <div className="bg-white border border-orange-200 rounded-lg p-4 mb-6">
        <p className="text-sm text-gray-700">
          ...
          <strong className="text-orange-600"> 개발자 개입 없이 어드민에서 직접 레이아웃을 조절</strong>할 수 있도록
            ...
            <tbody>
              {[
                ...
                <tr key={i} className="border-b border-gray-100">
                  ...
                  <td className="py-2">
                    <select className="text-xs border rounded px-2 py-1 bg-orange-50 border-orange-300 text-orange-700">
      ...
      <div className="grid grid-cols-3 gap-4 mb-4">
        {[
          ...
          <div key={item.size} className="text-center">
            <div className="text-sm font-bold text-orange-600 mb-2">{item.size}</div>
            <div className="h-8 bg-orange-100 rounded border border-orange-200 mx-auto" style={{ width: item.width, minWidth: '40px' }}></div>
      ...
      <div className="bg-green-50 border border-green-200 rounded-lg p-4">
        <p className="text-sm text-green-800 text-center">
          ...
          <br />
          <span className="text-green-600">프론트엔드 배포 없이 레이아웃 변경 완료</span>
        </p>
      </div>
    </div>
  </>
  ...
  <>
    ...
    <div className="grid md:grid-cols-3 gap-4 mb-8">
      {[
        ...
        <div key={util.name} className="bg-gray-50 rounded-lg p-4">
          <div className="font-medium text-gray-900 mb-2">{util.name}</div>
          <p className="text-xs text-gray-600">{util.desc}</p>
          <div className="mt-2 text-xs font-mono text-blue-600">{util.ret}</div>
        </div>
      ))}
    </div>
  </>
...
const DynamicFormConclusion = () => (
  <div className="bg-gradient-to-r from-gray-900 to-gray-800 rounded-xl p-6 text-white">
    ...
    <div className="grid md:grid-cols-3 gap-4 text-sm">
      {[
        { value: "0줄", desc: "새 프로젝트 시 프론트 코드 수정", color: "blue" },
        { value: "10+", desc: "동일 컴포넌트로 처리하는 페이지 수", color: "green" },
        { value: "100%", desc: "프로젝트 간 UX 일관성", color: "purple" },
      ].map((item) => (
